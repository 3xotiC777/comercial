...
const redirectUri = window.location.hostname.endsWith("github.io")
  ? `${window.location.origin}/comercial/auth.html`
...
const aliases: Record<string, string[]> = {
  ...
  attachment: ["Archivo", "Adjunto", "Insumo"],
  resolution: [
    "Solución",
    "Solucion",
    "Respuesta",
...
const analystAliases: Record<string, Analyst> = {
  diego: "Diego Montoya",
  "diego montoya": "Diego Montoya",
  miguel: "Miguel Cabezas",
  "miguel cabezas": "Miguel Cabezas",
  rony: "Rony Rodriguez",
  "rony rodriguez": "Rony Rodriguez",
};
export const normalizeAnalyst = (value: string): Analyst =>
  analystAliases[value.trim().toLowerCase()] || "";
...
async function token(interactive = false) {
  ...
  if (!account && interactive)
    account =
      (await msal.loginPopup({ scopes, overrideInteractionInProgress: true }))
        .account || undefined;
  if (!account) return null;
  msal.setActiveAccount(account);
  try {
    return (
      await msal.acquireTokenSilent({ scopes, account, forceRefresh: true })
    ).accessToken;
  } catch {
    return interactive
      ? (
...
async function graph(
  ...
  if (!response.ok) {
    ...
    const resource = path.startsWith("/me")
      ? "perfil de Microsoft"
      : path.includes("/drives")
        ...
        : path.includes("/lists")
          ...
          : "sitio reportingdn";
    throw new Error(
      `Microsoft Graph respondió ${response.status} al acceder a ${resource}.${detail ? ` ${detail}` : ""}`,
    );
  }
  return response.status === 204 ? null : response.json();
}
function field(columns: Map<string, string>, key: string) {
  return (
    aliases[key].map((x) => columns.get(norm(x))).find(Boolean) ||
    fallbackFields[key]
  );
}
function value(
...
function set(
  ...
  const name = field(columns, key);
  if (name) fields[name] = data;
}

async function getContext(accessToken: string) {
  if (context) return context;
  const site = await graph(
    "/sites/dichterneiracorp.sharepoint.com:/sites/reportingdn",
    accessToken,
  );
  ...
  const list = lists.value.find(
    (x: { displayName: string }) =>
      norm(x.displayName) === "comercialplaneacion",
  );
  if (!list) throw new Error("No encontré la lista “Comercial planeacion”.");
  const columnsResponse = await graph(
    `/sites/${site.id}/lists/${list.id}/columns?$select=name,displayName,readOnly,hidden`,
    accessToken,
  );
  ...
  const drive = drives.value.find(
    (x: { name: string }) => norm(x.name) === "comercialplaneacionproyecto",
  );
  if (!drive)
    throw new Error(
      "No encontré la biblioteca “Comercial planeacion proyecto”.",
    );
  const writableColumns = columnsResponse.value.filter(
    (x: { name: string; readOnly?: boolean; hidden?: boolean }) =>
      !x.readOnly && !x.hidden && x.name !== "DocIcon",
  );
  const columns = new Map<string, string>(
    writableColumns.map((x: { name: string; displayName: string }) => [
      norm(x.displayName),
      x.name,
    ]),
  );
  context = { siteId: site.id, listId: list.id, driveId: drive.id, columns };
  return context;
}

export async function signIn() {
  const accessToken = await token(true);
  if (!accessToken) throw new Error("No se pudo iniciar sesión con Microsoft.");
...
async function discoverAttachmentNames(
  accessToken: string,
  c: { driveId: string },
  tickets: Ticket[],
) {
  ...
  try {
    const root = await graph(
      `/drives/${c.driveId}/root/children?$select=name,folder&$top=999&$expand=children($select=name,file,folder)`,
      accessToken,
    );
    for (const folder of root.value as Array<{
      name: string;
  ...
  const unresolved = tickets
      .filter((ticket) => !ticket.attachment_name && !found.has(ticket.id))
      .slice(0, 100),
    chunks: Array<Ticket[]> = [];
  for (let index = 0; index < unresolved.length; index += 20)
    chunks.push(unresolved.slice(index, index + 20));
  const batches = await Promise.all(
    chunks.map(async (chunk) => {
      try {
        return {
          chunk,
          response: await graph("/$batch", accessToken, {
            method: "POST",
            body: JSON.stringify({
              requests: chunk.map((ticket, index) => ({
                id: String(index),
                method: "GET",
                url: `/drives/${c.driveId}/root:/${encodeURIComponent(ticket.id)}:/children?$select=name,file,folder`,
              })),
            }),
          }),
        };
  ...
  for (const batch of batches) {
    if (!batch) continue;
    for (const response of batch.response.responses as Array<{
      id: string;
      status: number;
      body?: { value?: Array<{ name: string; file?: unknown }> };
    }>) {
      ...
      (item: {
        ...
        return {
          ...
          status: (value(item.fields, c.columns, "status") ||
            "Pendiente") as Status,
          assignee: normalizeAnalyst(value(item.fields, c.columns, "assignee")),
          attachment_name:
            value(item.fields, c.columns, "attachment") || undefined,
          resolution: value(item.fields, c.columns, "resolution") || undefined,
          resolution_files: resolutionFiles
            ? resolutionFiles.split("; ").filter(Boolean)
            : undefined,
          completed_at:
            value(item.fields, c.columns, "completedAt") || undefined,
        };
      },
    )
...
async function uploadRequestFile(
  ...
  onProgress?.(0);
  const encodedPath = [ticketId, file.name]
  ...
  if (file.size <= resumableUploadThreshold) {
    ...
    if (!response.ok)
      throw new Error(
        `No se pudo cargar “${file.name}”. La solicitud no fue creada.`,
      );
    onProgress?.(100);
    return;
  }
  ...
  if (!uploadUrl)
    throw new Error("SharePoint no pudo iniciar la carga del archivo grande.");
  ...
  for (let start = 0; start < file.size; start += uploadChunkSize) {
    ...
    if (!response?.ok) {
      ...
      throw new Error(
        `No se pudo cargar “${file.name}”. La solicitud no fue creada.${detail ? ` ${detail}` : ""}`,
      );
    }
...
export async function downloadTicketAttachment(ticket: Ticket) {
  if (!ticket.attachment_name)
    throw new Error("Este ticket no tiene un archivo para descargar.");
  const downloadWindow = window.open("about:blank", "_blank");
  if (!downloadWindow)
    throw new Error(
      "El navegador bloqueó la descarga. Permite las ventanas emergentes para esta página e inténtalo nuevamente.",
    );
  downloadWindow.document.title = "Preparando descarga";
  downloadWindow.document.body.textContent = `Preparando ${ticket.attachment_name}…`;
  try {
    const accessToken = await token(true);
    if (!accessToken) throw new Error("Debes iniciar sesión con Microsoft.");
    const c = await getContext(accessToken),
      path = [ticket.id, ticket.attachment_name]
        .map((part) => encodeURIComponent(part))
        .join("/");
    let item = await graph(
        `/drives/${c.driveId}/root:/${path}?$select=id,name,webUrl,@microsoft.graph.downloadUrl`,
        accessToken,
      ),
    ...
    if (!downloadUrl && item?.id) {
      item = await graph(
        `/drives/${c.driveId}/items/${encodeURIComponent(item.id)}?$select=id,name,webUrl,@microsoft.graph.downloadUrl`,
        accessToken,
      );
      downloadUrl = String(item?.["@microsoft.graph.downloadUrl"] || "");
      webUrl = String(item?.webUrl || webUrl);
    }
    if (downloadUrl) {
    ...
    if (webUrl) {
      const fallback = new URL(webUrl);
      fallback.searchParams.delete("web");
      fallback.searchParams.set("download", "1");
      downloadWindow.location.replace(fallback.toString());
      return;
    }
    throw new Error(
      "SharePoint encontró el archivo, pero no entregó una ruta para abrirlo.",
    );
  } catch (error) {
...
export async function updateTicket(ticket: Ticket, patch: Partial<Ticket>) {
  ...
  const statusField = field(c.columns, "status"),
    assigneeField = field(c.columns, "assignee");
  ...
  if (
    patch.status &&
    statusField &&
    String(updated?.[statusField] ?? "") !== patch.status
  )
    throw new Error("SharePoint no confirmó el cambio de estado.");
  if (
...
async function ensureFolder(
  ...
  const encodedParent = parentPath
      ...
      .map((part) => encodeURIComponent(part))
      .join("/"),
    path = [encodedParent, encodeURIComponent(name)].filter(Boolean).join("/"),
    endpoint = parentPath
      ? `/drives/${driveId}/root:/${encodedParent}:/children`
      : `/drives/${driveId}/root/children`;
  try {
    await graph(`/drives/${driveId}/root:/${path}`, accessToken);
    return;
  } catch (error) {
    if (!(error instanceof Error) || !error.message.includes("respondió 404"))
      throw error;
  }
  await graph(endpoint, accessToken, {
    method: "POST",
    body: JSON.stringify({
      name,
      folder: {},
...
async function uploadSolutionFile(
  accessToken: string,
  driveId: string,
  ticketId: string,
  folder: "Adjuntos" | "Inline",
  file: File,
) {
  const response = await fetch(
    `https://graph.microsoft.com/v1.0/drives/${driveId}/root:/${encodeURIComponent(ticketId)}/Solucion/${folder}/${encodeURIComponent(file.name)}:/content`,
    {
      method: "PUT",
      headers: {
        Authorization: `Bearer ${accessToken}`,
  ...
  if (!response.ok) {
    ...
    throw new Error(
      `No se pudo cargar “${file.name}”.${detail ? ` ${detail}` : ""}`,
    );
  }
}
...
export async function finalizeTicket(
  ...
  if (!resolutionField)
    throw new Error(
      "Falta la columna “Solución” en la lista Comercial planeacion. Créala como “Varias líneas de texto” y vuelve a intentarlo.",
    );

  await ensureFolder(accessToken, c.driveId, "", ticket.id);
  await ensureFolder(accessToken, c.driveId, ticket.id, "Solucion");
  await ensureFolder(
    accessToken,
    c.driveId,
    `${ticket.id}/Solucion`,
  ...
  await ensureFolder(accessToken, c.driveId, `${ticket.id}/Solucion`, "Inline");
  for (const file of files)
    await uploadSolutionFile(
      accessToken,
      c.driveId,
      ticket.id,
      "Adjuntos",
  ...
  for (const file of inlineFiles)
    await uploadSolutionFile(accessToken, c.driveId, ticket.id, "Inline", file);

  const endpoint = `/sites/${c.siteId}/lists/${c.listId}/items/${ticket.spId}/fields`,
  ...
  try {
    ...
    throw new Error(
      `No se pudo guardar la solución en SharePoint.${error instanceof Error ? ` ${error.message}` : ""}`,
    );
  }
  ...
  if (completedAtField) {
    try {
      await graph(endpoint, accessToken, {
        method: "PATCH",
        body: JSON.stringify({ [completedAtField]: completedAt }),
      });
    } catch {
      /* La fecha es opcional: no debe bloquear el envío de la solución. */
    }
  }
  ...
  if (statusField) {
    try {
      updated = await graph(endpoint, accessToken, {
        method: "PATCH",
        body: JSON.stringify({ [statusField]: "Finalizado" }),
      });
    } catch (error) {
      throw new Error(
        `La solución se guardó, pero no se pudo cambiar el estado a Finalizado.${error instanceof Error ? ` ${error.message}` : ""}`,
      );
    }
    if (String(updated?.[statusField] ?? "") !== "Finalizado")
      throw new Error("SharePoint no confirmó el cierre del ticket.");
  }
  return {
